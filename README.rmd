...
Collisions occur in colormaps when two different data values map to the same color. One way to address this issue is through alpha blending. In alpha blending, instead of assigning a single color to each data value, a combination of colors is used. This is achieved by blending the colors of adjacent data points using an alpha channel. The alpha channel controls the degree to which each color contributes to the final blended color. By blending colors, it becomes less likely that different data values will map to the same color, thus reducing the likelihood of collisions.

$$ result = { (1 - α) * color_{1} + α * color_{2}} $$

Here, color1 and color2 are used to represent the inputs to the bivaraite colormap.
α parameter on the other hand is used to calculate the weightage of each of the two variables.
